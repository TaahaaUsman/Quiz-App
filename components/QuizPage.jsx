...
import { FaExclamation, FaCalendarCheck, FaSave, FaAngleLeft, FaAngleRight, FaUser,FaAngleDoubleRight, FaAngleDoubleLeft } from "react-icons/fa";
...
const QuizPage = ({ quiz, courseDetails, courseId }) => {
  ...
  const handleOptionChange = (index) => {
    ...
    setSubmissionStatus(null); // Reset submission status when a new option is selected
  };
    ...
    return (
      <div className="flex flex-col justify-center items-center h-screen bg-gradient-to-br from-blue-50 to-white text-gray-700 px-4">
        ...
        <p className="text-lg text-gray-600 mb-6 text-center max-w-md">
          It looks like this course doesn't have any <span className="text-blue-500 underline"></span> MCQs or questions added yet. Please check back later or explore other available courses.
        </p>
        <Link href={`/courses/${courseId}`} className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2 rounded-lg transition duration-300 shadow-md">
          Back to Course
        </Link>
      </div>
    );
      ...
      <div className={`h-[13%] md:h-[10%] text-sm`}>
        <div className="w-full h-full bg-gray-200">
          <div className="h-full flex flex-col md:flex-row justify-between items-center px-4 md:px-0">
            ...
            <div className={`mt-2 md:mt-0 flex flex-col md:flex-row ${isMobile? "": "gap-4"} w-full md:w-auto`}>
              ...
              <div className="flex w-full md:w-auto">
                ...
                {isMobile? <div className="flex">
                  ...
                  <button
                  onClick={handleNext}
                  className={`bg-[#c19028d5] border-r rounded-l-md text-black py-2 w-1/2 md:w-24 border-black flex items-center justify-center gap-1 hover:bg-opacity-80 active:scale-95 transition`}
                >
                  <span>Next</span>
                  <FaAngleRight style={{ fontSize: '1rem', color: 'black' }} />
                </button>
              ...
              {isMobile? <div></div> : <div>
                ...
                <button
                  onClick={handleNext}
                  className={`bg-[#c19028d5] ${isMobile? "border-r rounded-l-md" : "border-l rounded-r-md"} text-black py-2 w-1/2 md:w-24 border-black flex items-center justify-center gap-1 hover:bg-opacity-80 active:scale-95 transition`}
                >
                  <span>Next</span>
                  <FaAngleRight style={{ fontSize: '1rem', color: 'black' }} />
                </button>
              ...
              <button
                onClick={handleSubmit}
                className={`bg-[#34bfa3cd] ${isMobile? "border-l border-black rounded-r-md" : "rounded-md ml-4"} text-white px-4 py-2 flex items-center justify-center gap-1 hover:bg-opacity-80 active:scale-95 transition`}
              >
                <FaSave style={{ fontSize: '1rem', color: 'white' }} />
                <span>Save</span>
              </button>
